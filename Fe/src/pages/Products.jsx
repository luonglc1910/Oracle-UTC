...
export default function Products () {
  ...
  const filtered = products.filter(p => {
    ...
    const matchSearch =
      !search || p.TEN_SP.toLowerCase().includes(search.toLowerCase())
    ...
    <>
      ...
      <div className='container' style={{ padding: '32px 20px' }}>
        ...
        <div className='filter-bar'>
          <button
            className={`filter-btn ${!activeDm ? 'active' : ''}`}
        ...
        <div
          style={{ marginBottom: 16, color: 'var(--text-light)', fontSize: 14 }}
        >
            ...
            {filtered.map(p => (
              <div key={p.MA_SP} className='product-card'>
                ...
                <div className='product-body'>
                  ...
                  {p.TRONG_LUONG && (
                    <div className='product-weight'>⚖️ {p.TRONG_LUONG}g</div>
                  )}
                  <div className='product-price-row'>
                    ...
                    <span
                      className={`product-stock ${
                        p.SO_LUONG > 0 ? 'in-stock' : 'out-stock'
                      }`}
                    >
                      {p.SO_LUONG > 0 ? `Còn ${p.SO_LUONG}` : 'Hết hàng'}
                    </span>
                  </div>
