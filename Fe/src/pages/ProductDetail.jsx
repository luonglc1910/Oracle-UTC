...
export default function ProductDetail () {
  ...
  const handleReview = async e => {
    ...
    try {
      await danhGiaApi.create({
        ...reviewForm,
        ma_sp: id,
        ma_kh: reviewForm.ma_kh || user?.MA_KH
      })
      ...
      showToast('Có lỗi khi gửi đánh giá', '❌')
    }
  }
  ...
  const avgRating = reviews.length
    ? (
        reviews.reduce((s, r) => s + r.DIEM_DANH_GIA, 0) / reviews.length
      ).toFixed(1)
    : null
    ...
    <div className='detail-page'>
      ...
      <div className='detail-layout'>
        ...
        <div className='detail-info'>
          ...
          {avgRating && (
            <div style={{ color: 'var(--gold)', margin: '8px 0' }}>
              {'⭐'.repeat(Math.round(avgRating))} {avgRating} ({reviews.length}{' '}
              đánh giá)
            </div>
          )}
          ...
          <div className='detail-meta'>
            {product.TRONG_LUONG && (
              <div className='meta-item'>
                <div className='meta-val'>{product.TRONG_LUONG}g</div>
                <div className='meta-key'>Trọng lượng</div>
              </div>
            )}
            <div className='meta-item'>
              <div className='meta-val'>{product.TON_KHO || 0}</div>
              <div className='meta-key'>Tồn kho</div>
            </div>
            <div className='meta-item'>
              <div className='meta-val'>{product.TEN_NCC || '—'}</div>
              <div className='meta-key'>Nhà cung cấp</div>
            </div>
          </div>
          ...
          <div className='qty-selector'>
            ...
            <div className='qty-control'>
              ...
              <button
                className='qty-btn'
                onClick={() => setQty(q => Math.min(product.TON_KHO, q + 1))}
              >
                +
              </button>
            </div>
          ...
          <div key={r.MA_DG} className='review-card'>
            <div className='review-header'>
              <div>
                ...
                <span className='review-stars' style={{ marginLeft: 8 }}>
                  {'⭐'.repeat(r.DIEM_DANH_GIA)}
                </span>
              </div>
        ...
        <div className='review-form' style={{ marginTop: 24 }}>
          ...
          <form onSubmit={handleReview}>
            <div className='form-group'>
              ...
              <input
                type='number'
                placeholder='Nhập mã KH của bạn'
                value={reviewForm.ma_kh}
                onChange={e =>
                  setReviewForm(f => ({ ...f, ma_kh: e.target.value }))
                }
                required
              />
            </div>
            <div className='form-group'>
              ...
              <select
                value={reviewForm.diem_danh_gia}
                onChange={e =>
                  setReviewForm(f => ({
                    ...f,
                    diem_danh_gia: Number(e.target.value)
                  }))
                }
              >
                {[5, 4, 3, 2, 1].map(n => (
            ...
            <div className='form-group'>
              ...
              <textarea
                ...
                onChange={e =>
                  setReviewForm(f => ({ ...f, noi_dung: e.target.value }))
                }
                required
                style={{
                  width: '100%',
